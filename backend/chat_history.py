...
def _get_messages_query_with_ids(table_name: str) -> sql.Composed:
    """Make a SQL query to get messages for a given session."""
    return sql.SQL(
        "SELECT message "
        "FROM {table_name} "
        "WHERE session_id = %(session_id)s AND message_id = ANY(%(message_ids)s) "
        "ORDER BY id;"
    ).format(table_name=sql.Identifier(table_name))
...
def _insert_message_query(table_name: str) -> sql.Composed:
    ...
    return sql.SQL(
        "INSERT INTO {table_name} (session_id, message_id, message) VALUES (%s, %s, %s)"
    ).format(table_name=sql.Identifier(table_name))


class PostgresChatMessageHistoryWithId(BaseChatMessageHistory):
    def __init__(
        self,
        table_name: str,
        session_id: str,
        ...
        *,
        ...
        if not sync_connection and not async_connection:
            raise ValueError(
                "Must provide sync_connection or async_connection")
    ...
    def add_messages(self, messages: Sequence[BaseMessage]) -> None:
        ...
        values = [
            (self._session_id, message.id, json.dumps(message_to_dict(message)))
    ...
    def get_messages_by_ids(self, message_ids: list[str]) -> List[BaseMessage]:
        """Retrieve messages from the chat message history."""
        if self._connection is None:
            raise ValueError(
                "Please initialize the PostgresChatMessageHistory "
        ...
        query = _get_messages_query_with_ids(self._table_name)

        with self._connection.cursor() as cursor:
            cursor.execute(
                query, {"session_id": self._session_id, "message_ids": message_ids})
            items = [record[0] for record in cursor.fetchall()]
    ...
    async def aget_messages_by_ids(self, message_ids: list[str]) -> List[BaseMessage]:
        """Retrieve messages from the chat message history."""
        if self._aconnection is None:
            raise ValueError(
                "Please initialize the PostgresChatMessageHistory "
        ...
        query = _get_messages_query_with_ids(self._table_name)
        async with self._aconnection.cursor() as cursor:
            await cursor.execute(query, {"session_id": self._session_id, "message_ids": message_ids})
            items = [record[0] for record in await cursor.fetchall()]
